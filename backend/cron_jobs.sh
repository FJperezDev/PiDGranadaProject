...
# CORRECCIÓN: Usamos un quoting seguro para que el shell pase la cadena completa a python
GENERATE_CMD="python /app/manage.py shell -c \"from apps.audit.utils import generate_excel_backup; generate_excel_backup(is_auto=True)\""
...
if [[ "$(date +%d)" == "01" ]]; then
    ...
    # Ejecutamos el comando Python corregido
    $GENERATE_CMD
