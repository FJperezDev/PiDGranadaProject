...
export default function QuestionWizardModal({ visible, onClose, onSaveSuccess, editingQuestion }) {
  ...
  // --- Datos del Paso 2 y 3: Contenido ---
  const [statementES, setStatementES] = useState('');
  const [statementEN, setStatementEN] = useState('');
  
  // Estado para rastrear IDs de respuestas que se deben borrar al guardar
  ...
  // Cargar topics cuando cambia la asignatura (solo si hay seleccionada)
  useEffect(() => {
    if (selectedSubject) {
  ...
  // Cambio manual de asignatura: limpia los temas seleccionados
  ...
  const loadEditingData = async () => {
    ...
    try {
      ...
      const formattedAnswers = apiAnswers.map(a => ({
          ...
          text_es: a.text_es || '',
          text_en: a.text_en || '',
          tempId: a.id // Guardamos el ID real
      }));
  ...
  const handleRemoveAnswer = (index) => {
    ...
    // Si tiene ID real, lo marcamos para borrar en la BD
  ...
  const handleNext = () => {
      if (step === 1) {
        ...
        if (!statementES.trim()) {
            Alert.alert("Error", "El enunciado en español es obligatorio");
            return;
        }
        if (!answers.some(a => a.is_correct)) {
            Alert.alert("Error", "Marca al menos una respuesta como correcta");
            return;
        }
        if (answers.some(a => !a.text_es.trim())) {
            Alert.alert("Error", "Todas las opciones deben tener texto en español");
            return;
        }
        setStep(3);
      }
  ...
  const handleSubmit = async () => {
    ...
    if (answers.some(a => !a.text_en.trim())) {
        Alert.alert("Error", "Todas las opciones deben tener traducción al inglés");
        return;
    }
    ...
    try {
        ...
        if (editingQuestion) {
            ...
            // NOTA: El backend 'update' usa "topics" en lugar de "topics_titles"
            const qData = {
                type: questionType,
                statement_es: statementES,
                statement_en: statementEN,
                topics: selectedTopicTitles, // IMPORTANTE: 'topics' para update según tu backend
                concepts: [] 
            };
            await updateQuestion(questionId, qData);

            // 2. Eliminar respuestas borradas
            if (deletedAnswerIds.length > 0) {
                const deletePromises = deletedAnswerIds.map(ansId => 
                    deleteAnswer(questionId, ansId)
                );
                await Promise.all(deletePromises);
            }

            // 3. Actualizar o Crear respuestas
            const upsertPromises = answers.map(ans => {
                ...
                if (ans.id) {
                    console.log("Processing answer:", ans.id);
                    // Tiene ID -> PUT /questions/<id>/answers/<ans_id>/
            ...
            const qData = {
                ...
                statement_es: statementES,
                statement_en: statementEN,
                topics_titles: selectedTopicTitles, // IMPORTANTE: 'topics_titles' para create según tu backend
                concepts: [] 
            };
  ...
  const renderStep1 = () => (
    <View style={styles.stepContainer}>
        <Text style={styles.sectionHeader}>Contexto de la Pregunta</Text>
        ...
        <ScrollView style={styles.topicsScroll} nestedScrollEnabled={true}>
            <View style={styles.topicsGrid}>
                {topics.length === 0 && <Text style={{color: 'gray', padding: 10}}>No hay temas en esta asignatura.</Text>}
        ...
        <View style={styles.pickerWrapper}>
            <Picker selectedValue={questionType} onValueChange={val => setQuestionType(val)}>
  ...
  const renderStep2 = () => (
    <ScrollView style={styles.stepContainer}>
        <Text style={styles.sectionHeader}>Versión en Español</Text>
        <Text style={styles.label}>Enunciado (ES)</Text>
        <TextInput 
            style={[styles.input, { height: 80 }]} 
            multiline 
            placeholder="Escribe la pregunta en español..." 
            value={statementES} 
            onChangeText={setStatementES} 
        />

        <Text style={styles.label}>Respuestas (ES)</Text>
        <Text style={styles.subLabel}>Marca el switch si es correcta.</Text>
        
        {answers.map((ans, index) => (
            <View key={ans.id || ans.tempId} style={styles.answerRow}>
                 <Switch 
                    value={ans.is_correct} 
                    onValueChange={() => handleCorrectChange(index)} 
                    trackColor={{ false: "#767577", true: COLORS.success || "#4cd964" }}
                 />
                 <TextInput 
                    style={styles.answerInput} 
                    placeholder={`Opción ${index + 1} (Español)`} 
                    value={ans.text_es} 
                    onChangeText={(text) => handleAnswerChangeES(text, index)}
                 />
  ...
  const renderStep3 = () => (
    <ScrollView style={styles.stepContainer}>
        <Text style={styles.sectionHeader}>Versión en Inglés</Text>
        <Text style={styles.label}>Enunciado (EN)</Text>
        <Text style={styles.helperText}>Original: "{statementES}"</Text>
        <TextInput 
            style={[styles.input, { height: 80 }]} 
            multiline 
            placeholder="Type the question in English..." 
            value={statementEN} 
            onChangeText={setStatementEN} 
        />

        <Text style={styles.label}>Respuestas (EN)</Text>
        <Text style={styles.helperText}>Traduce las opciones definidas anteriormente.</Text>

        {answers.map((ans, index) => (
            <View key={ans.id || ans.tempId} style={styles.answerRowTranslation}>
                 <View style={styles.originalTextContainer}>
                    <Text style={styles.originalTextLabel}>ES: {ans.text_es}</Text>
                 ...
                 <TextInput 
                    ...
                    value={ans.text_en} 
                    onChangeText={(text) => handleAnswerChangeEN(text, index)}
                 />
            </View>
        ))}
    </ScrollView>
      ...
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          <Text style={styles.modalTitle}>
            {editingQuestion ? "Editar Pregunta" : "Nueva Pregunta"} ({step}/3)
          </Text>
            ...
            <View style={{flexDirection: 'row', gap: 10}}>
                ...
                {step < 3 ? (
                    ...
                    <TouchableOpacity style={styles.saveBtn} onPress={handleSubmit} disabled={loading}>
                        ...
                        <Text style={styles.nextBtnText}>{loading ? "Guardando..." : "Guardar"}</Text>
                    </TouchableOpacity>
                )}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  answerInput: { flex: 1, borderWidth: 1, borderColor: '#eee', borderRadius: 5, padding: 8, backgroundColor: '#f9f9f9', width: '100%' },
